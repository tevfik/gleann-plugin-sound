...
// Register registers the hotkey on Windows via RegisterHotKey
...
func (hk *Hotkey) Register() error {
	...
	hk.platform = &windowsPlatform{extHk: extHk}
	log.Println("[hotkey] registered via RegisterHotKey (Windows)")
...
// Unregister unregisters the hotkey on Windows.
func (hk *Hotkey) Unregister() error {
	...
	if p, ok := hk.platform.(*windowsPlatform); ok {
		p.extHk.Unregister()
	}
...
func modsToExt(mods []Modifier) ([]exthotkey.Modifier, error) {
	...
	for _, m := range mods {
		...
		case ModAlt:
			out = append(out, exthotkey.ModAlt)
		case ModSuper:
			out = append(out, exthotkey.ModWin)
		default:
